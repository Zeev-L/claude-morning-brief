...
{"sessionId":"<מתוך הקלט>","title":"<כותרת נושא קצרה (2-5 מילים) שתשמש רק אם לסשן אין שם>","about":"משפט אחד קצר: על מה הסשן הזה בכללי (הנושא/המטרה)","did":["1-3 נקודות קצרות בעברית של מה נעשה"],"stopped":"משפט אחד: הפעולה האחרונה שנעשתה / נקודת העצירה","next":"משפט אחד: הצעד הבא (הסק מההקשר; אם לא ברור כתוב: לא ברור, החלט כשתחזור)"}

כללים: עברית, תמציתי מאוד, ענייני. שדה about הוא תיאור-על של מטרת הסשן (לא פעולה בודדת). הסתמך אך ורק על userMsgs ו-lastAssistant של אותו סשן. אל תמציא. החזר רק את ה-JSON.
